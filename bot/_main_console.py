...
admin_key = os.environ.get('OPENAI_ADMIN_KEY', None)
if admin_key:
    openai_config['admin_key'] = admin_key
...
telegram_config = {
    ...
    'budget': {
        'admin': float(os.environ.get('ADMIN_BUDGETS', 20)),
        'moder': float(os.environ.get('MODER_BUDGETS', 10)),
        'user': float(os.environ.get('USER_BUDGETS', 1)),
        'guest': float(os.environ.get('GUEST_BUDGET', 0)),
    },
    'budget_period': os.environ.get('BUDGET_PERIOD', 'daily').lower(),
